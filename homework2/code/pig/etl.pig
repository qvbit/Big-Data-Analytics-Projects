...
deadevents = FOREACH deadfilt GENERATE events::patientid, events::eventid, events::value, mortality::label, DaysBetween(SubtractDuration(mortality::mtimestamp, 'P30D'), events::etimestamp);
...
fillna = FOREACH eventswithmort GENERATE events::patientid AS patientid, events::eventid AS eventid, events::value AS value, events::etimestamp AS etimestamp, (mortality::label IS NULL ? 0:1) AS label;
alivefilt = FILTER fillna BY (label == 0);
alivegrouped = GROUP alivefilt BY patientid;
aliveindex = FOREACH alivegrouped GENERATE group AS patientid, MAX(alivefilt.etimestamp) AS index; -- "group" is what we grouped by (i.e. patientid)
alivejoin = JOIN alivefilt BY patientid, aliveindex BY patientid;
aliveevents = FOREACH alivejoin GENERATE alivefilt::patientid AS patientid, alivefilt::eventid AS eventid, alivefilt::value AS value, alivefilt::label AS label, DaysBetween(aliveindex::index, alivefilt::etimestamp) AS time_difference;
...
dropnull = FILTER combined BY value IS NOT NULL;
filtered = FILTER dropnull BY (time_difference>=0) AND (time_difference<=2000);
...
featureswithid = FOREACH grouped GENERATE group.$0 AS patientid, group.$1 AS eventid, COUNT(filtered.value) AS featurevalue;
...
all_events = FOREACH featureswithid GENERATE eventid;
distinct_all_events = DISTINCT all_events;
sorted_distinct_all_events = ORDER distinct_all_events BY eventid;
index_and_event = RANK sorted_distinct_all_events;
all_features = FOREACH index_and_event GENERATE ($0-1) AS idx, $1;
...
features = FOREACH joined GENERATE featureswithid::patientid AS patientid, all_features::idx AS idx, featureswithid::featurevalue AS featurevalue;
...
grouped = GROUP features BY idx;
maxvalues = FOREACH grouped GENERATE group AS idx, MAX(features.featurevalue) AS maxvalue;
...
id_label = FOREACH filtered GENERATE patientid, label;
labels = DISTINCT id_label;
